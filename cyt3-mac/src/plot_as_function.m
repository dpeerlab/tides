function [X, Y_vals] = plot_as_function(x, Y, varargin)
...
for i=1:length(varargin)-1
    ...
    elseif (strcmp(varargin{i}, 'make_plot'))
        make_plot = 1;
...
% function [X, Y_vals, Y_errs] = plot_as_function(x, Y, varargin)
% % plots Y (NxM) as a function of vector x (N length).  
% % an average of points in Y is computed for 'num_locs' along vector x.
% %
% % 'labels': a string cell array to enter for the legend
...
%     Y_vals = Y_vals./repmat(prctile((y_vals_all), 100, 1),size(Y_vals,1),1);
...
% 
% if make_plot == 1
%     plot(X, Y_vals(:, 1), 'LineWidth', 2, 'Color', cols);    
%     %plot(X, Y_vals(:, 1), 'LineWidth', 4, 'Color', cols);    
%     if (size(Y, 2)> 1)
%         for col=2:size(Y, 2)
%             hold on;
...
%             cols,'linestyle','none','facealpha',.1);
%     end
% end
% 
% if (any(branch))
...
% %     imagesc(X, ca(3):0.04:ca(3)+0.05, dens, [0, max(dens)]);
% %     colorbar;
% %     axis(ca);
% %     catch e
% %     disp(getReport(e,'extended'));
