...
%             ylim([0, Inf])
        ...
        function [DREMI_values, Traj_time, Vqq, meshX2d_ex, meshY2d, meshZ2d, meshX] = compute_windowed_DREMI_interpolate(obj, window_channel, channel1_name, channel2_name, steps, prob_threshold, num_partitions, varargin)
            
            call_at = 0.1; set_min = 0; num_bins_val = 128;
            if ~isempty(varargin)
                for i = 1:length(varargin)
...
%             maxz = max(obj.get_data(channel3_name));
        ...
        function [DREMI, estimated_points, meshX, meshY] = compute_twoD_dremi(obj, channel1_name, channel2_name, prob_threshold, num_partitions, varargin)
...
%             end
        ...
        function [conditional_mean, meshX, meshY, meshZ, estimated_points, Movie_ko_lagi, F_fitted, MSE, x,y_fit, Kd, conditional_estimated_points_grid, meshX2d, meshY2d, cellhist, edgesx, edgesy] = threeD_edge_visualize(obj, channel1_name, channel2_name, channel3_name, points_to_slice_at, varargin)
            ...
            draw_response = 0; set_min = 0; num_bins_val = 128;
            ...
            if yes_bandwidth == 1
                if set_min == 1
                    [ estimated_points, meshX, meshY, meshZ, conditional_mean, conditional_estimated_points_grid, meshX2d, meshY2d] = conditional(edge_data,128,'bandwidth', bandwidth, 'set_min');
                else
                ...
                if set_min ==1
                    [ estimated_points, meshX, meshY, meshZ, conditional_mean, conditional_estimated_points_grid, meshX2d, meshY2d] = conditional_min(edge_data,num_bins_val, 'set_min');
                else
                    [ estimated_points, meshX, meshY, meshZ, conditional_mean, conditional_estimated_points_grid, meshX2d, meshY2d] = conditional_min(edge_data,128);
                end
            end
